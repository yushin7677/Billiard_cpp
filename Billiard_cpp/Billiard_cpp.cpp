				...
				// Бьет компьютер
					...
					while (tryes < 20 && !(goodAngle && !otherBalls)) {
						...
						for (int i = 0; i < howManyBalls && !otherBalls; i++) {
							...
							float cx = coorOther.x - coor10.x;
							float cy = coorOther.y - coor10.y;
							float vx = coor10.x - coor1.x;
							float vy = coor10.y - coor1.y;
							float part;
							float D = -(cy * vx - cx * vy) * (cy * vx - cx * vy) + 400 * (vx * vx + vy * vy); // 400 = (2*10)**2
							if (D < 0) part = 100;
						...
						for (int i = 0; i < howManyBalls && !otherBalls; i++) {
							...
							float cx = coorOther.x - coor20.x;
							float cy = coorOther.y - coor20.y;
							float vx = coor20.x - coorHole.x;
							float vy = coor20.y - coorHole.y;
							float part;
							float D = -(cy * vx - cx * vy) * (cy * vx - cx * vy) + 400 * (vx * vx + vy * vy); // 400 = (2*10)**2
							if (D < 0) part = 100;
							else part = (-(cx * vx + cy * vy) - sqrt(D)) / (vx * vx + vy * vy);
					...
					if (hitBall == 0 && hitBoard < 3 && score[player - 1]) {
						...
						// Если ударил пользователь (или компьютер играет сам с собой), то ставит компьютер:
							...
							while (!canPut) {
								...
								coorComputer = { float{44.} + rand() % 712, float{44.} + rand() % 312 };
...
void begin1(ye::World& world) {
	...
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 3; j++) {
			...
			world.createScene({ float{15.} + j * 385, float{15.} + i * 370 });
	...
	for (int i = 0; i < 5; i++) {
		for (int j = 0; j < (5 - i); j++) {
			world.spawnActor({ float{200.} + i * 25, float{150.} + i * float{12.5} + j * float{25.} }, { 0.,0. });
