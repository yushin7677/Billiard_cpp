...
namespace ye{
	...
	public:
		...
		// Переменные
		float n = 1; // Точность расчета коллизий
	...
	private:
		...
		void relationshipWorld(Actor &a){ // Если происходит столкновение с миром, проверка коллизии закончена, если нет, то смещению присваивается скорость
			...
			float cx = a.coor.x;
			float cy = a.coor.y;
			float vx = a.v.x/(n*frame);
			float vy = a.v.y/(n*frame);
			float w,h;
			...
			if (!a1.collision && (!a2.collision || a2.typeName == "Scene") && a1.enableCollision && a2.enableCollision){
				...
				if (a1.isCircle && a2.isCircle){
					...
					if (vectorLength( (a1.coor + a1.v/n/frame) - (a2.coor + a2.v/n/frame) ) < dr){
						...
						float cx = a2.coor.x - a1.coor.x;
						float cy = a2.coor.y - a1.coor.y;
						float vx = v20.x - v10.x;
						float vy = v20.y - v10.y; 
						float part;
						if(vx != 0 || vy != 0){
							float D = -(cy*vx - cx*vy)*(cy*vx - cx*vy) + dr*dr*(vx*vx + vy*vy);
							if (D < 0) D = 0;
						...
						if (vectorLength(v10) != 0){
							float cos1 = v10*vOs/( vectorLength(v10)*vectorLength(vOs) );
						...
						if(vectorLength(v20) != 0){
							float cos2 = v20*vOs/( vectorLength(v20)*vectorLength(vOs) );
				...
				// Столкновение окружности и прямоугольника
					...
					float mCirc = aCirc->m, mRect = aRect->m;    // Массы
					float w = aRect->size.x, h = aRect->size.y;  // Размеры прямоугольника
					float r = aCirc->r;                          // Размеры круга
				...
				// Столкновение 2 прямоугольников
					...
					float m1 = a1.m, m2 = a2.m;
					float w1 = a1.size.x, w2 = a2.size.x, h1 = a1.size.y, h2 = a2.size.y; 
